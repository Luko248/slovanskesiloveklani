...
3. Replace hardcoded location on line 39:
   From: `Za Hasičskou Zbrojnicí, Pustiměř`
   To: `{getFullLocation()}`
...
<task type="auto">
  ...
  <action>
Remove or update hardcoded date/time from cs.json:

1. Remove hero.date key (line 12): "07.06.2026"
   - This is wrong (shows June 7, not June 13)
   - Date now comes from eventConfig

2. Update hero.time key (line 13):
   - Current: "Sobota od 10:00"
   - This is now unused (Hero gets time from eventConfig)
   - Remove this key as well

3. Remove hero.location key (line 14):
   - Current: "Pustiměřské Prusy, hasičská zbrojnice"
   - Location now comes from eventConfig
   - Remove this key

Keep remaining keys (nav.*, hero.title, hero.subtitle, hero.cta, about.*, footer.*) as they contain translatable text that doesn't change with event details.
...
Why: Hardcoded dates in i18n files create inconsistency. cs.json showed June 7 while Hero showed June 13. By removing these and using eventConfig, we have single source of truth.
  </action>
  <verify>
- cs.json has no date, time, or location keys under hero
- `bun run build` succeeds
- No TypeScript errors about missing translation keys (Hero no longer uses these keys)
  </verify>
  <done>
cs.json cleaned of hardcoded event dates, times, and location - all come from eventConfig
  </done>
</task>
