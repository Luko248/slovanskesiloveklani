...
```
  ...
  <verify>
- Code verification: `grep -A2 "getElementById('close-menu-btn')" src/components/Header.astro` shows closeBtn variable and addEventListener
- Code verification: `grep "getElementById('mobile-menu-btn')" src/components/Header.astro` shows correct selector
- Code verification: `grep "translate-x-full" src/components/Header.astro` shows toggle and add operations
- `bun run dev` and test on mobile viewport:
  ...
  4. Click "Pridat do kalendare" button - should download .ics file
- Desktop calendar button "Do Kalendare" should also download .ics file
  </verify>
  <done>
